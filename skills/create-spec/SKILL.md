...
- Structure `Acceptance Criteria / 验收标准` with the exact required subsections from the template: `Numerical Analysis / 数值分析`, `Precision Standards / 精度标准`, and `NumPy Compare Function / NumPy 精度比对函数`. Derive the numerical analysis from the PRD, mathematical semantics, functional reference functions, dtype rules, shape rules, and precision YAML. The numerical analysis must include the six required H4 subsections from the template: `Floating Point Error Analysis / 浮点误差分析`, `Stability Analysis / 稳定性分析`, `Conditioning / 条件数`, `Reduction Error Analysis / 归约误差分析`, `Mixed Precision Analysis / 混合精度分析`, and `Error Budget / 误差预算`. Fill every subsection with operator-specific engineering analysis. If a topic does not apply, explain why and still state a conclusion. End every H4 subsection with an explicit `Conclusion:` or `结论:` sentence that records the engineering decision. Write precision standards as a single YAML fenced block with top-level `scenarios`, where each scenario has `name`, `condition`, and non-empty `outputs`, and each output has `name`, `dtype`, numeric `atol`, numeric `rtol`, and `rationale`. Do not use Markdown tables for precision standards. Include one Python compare function with exact signature `def compare(actual_outputs, expected_outputs) -> Tuple[bool]:`; it must import or use NumPy, compare every output according to the YAML precision standards, and return a tuple of booleans aligned with output order or output entries.
...
First resolve the target repository root before choosing a manual validator path. Do not decide that the repository root is missing from a nested output directory such as `docs/mozi/<op-name-kebab-case>/`.
...
```bash
python3 skills/create-spec/scripts/validate_spec.py <provided-spec-path> --operator <OP_NAME>
```

When using an installed Mozi plugin in a target repository that does not contain this repository's root plugin source tree, run the bundled validator from this skill directory instead:
...
If validation fails, keep the generated or revised SPEC file and report the validator errors to the user. The validator is strict: unresolved `TBD` content, missing acceptance criteria subsections, Markdown-table precision standards, malformed precision YAML, missing output tolerances, invalid compare signatures such as `acutal_outputs`, and open issues make the SPEC incomplete. Report success only after the manual validator passes.
